...
path_models = "Pickles/"  #object serialization
# SVM
path_svm = path_models + 'best_svc.pickle'
with open(path_svm, 'rb') as data:
    svc_model = pickle.load(data)

# More specifically, TF-IDF is a method for term weighting, where each of the documents in a collection is characterized using weights assigned to the words (or terms) present in it.
# This makes it possible to rank the documents according to their estimated relevance to a search query.
# These concepts are still relevant to modern search engines and other kinds of text analysis.
...
def get_news_elpais():
    ...
    # Soup creation   (web scrapping data from web)
...
def get_news_bbc():
    ...
    # News identification
    coverpage_news = soup1.find_all('h2', class_='cluster-Latest__title1')
    len(coverpage_news)
    ...
    for n in np.arange(0, number_of_articles):
        ...
        # Getting the link of the article
        link = "https://www.beinsports.com/" + coverpage_news[n].find('a')['href']
        list_links.append(link)
        ...
        article_content = article.content
        soup_article = BeautifulSoup(article_content, 'html.parser')
        body = soup_article.find_all('div', class_='plrm')
        x = body[0].find_all('p')
...
def scrape_and_predict(n_clicks, values):
    ...
    if 'BBC' in values:
        df_features = df_features.append(get_news_bbc()[0])
        df_show_info = df_show_info.append(get_news_bbc()[1])
...
def update_barchart(jsonified_df):
    ...
    if 'BBC' in df_sum.index:

        df_sum_bbc = df_sum['BBC']
        x_bbc = ['Politics', 'Business', 'Entertainment', 'Sport', 'Tech', 'Other']
        y_bbc = [[df_sum_bbc['politics'] if 'politics' in df_sum_bbc.index else 0][0],
                 [df_sum_bbc['business'] if 'business' in df_sum_bbc.index else 0][0],
                 [df_sum_bbc['entertainment'] if 'entertainment' in df_sum_bbc.index else 0][0],
                 [df_sum_bbc['sport'] if 'sport' in df_sum_bbc.index else 0][0],
                 [df_sum_bbc['tech'] if 'tech' in df_sum_bbc.index else 0][0],
                 [df_sum_bbc['other'] if 'other' in df_sum_bbc.index else 0][0]]
    ...
    figure = {
        'data': [
            ...
            {'x': x_bbc, 'y':y_bbc, 'type': 'bar', 'name': 'BBC', 'marker': {'color': 'rgb(197, 223, 242)'}}
        ],
        'layout': {
